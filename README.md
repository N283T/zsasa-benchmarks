...
The adopted 0.9.0 refresh scope, completed measurements, scaling findings, and
deliberately unadopted plans are recorded in
[`docs/v0.9.0-refresh-summary.md`](docs/v0.9.0-refresh-summary.md).
...
The E. coli and Human PDB 0.9.0 refresh manifests rerun f32/f64
standard/bitmask at 10, 20, and 40 threads. Historical comparator and lower-thread
results remain associated with the completed 0.6.0 suite.
...
The planned E. coli AFDB mmCIF matrix is not adopted. Human mmCIF already covers
parser/I/O/worker scaling, while E. coli PDB covers the smaller batch workload;
`manifests/batch-ecoli-cif-zsasa-0.9.toml` is retained only as a documented
unexecuted plan.
...
commands and tool `--timing` component commands for parse/SASA timing. The current
PDB manifest reruns the four zsasa 0.9.0 precision/mode variants with PDBTools.jl
at 1, 4, 8, and 10 threads; historical FreeSASA and RustSASA PDB results remain
associated with the 0.6.0 suite. The same
runner also supports `manifests/single-file-mmcif-sample.toml`, which mirrors all
eight PDB subset structures and the PDB tool/thread matrix using uncompressed
mmCIF. Materialize those ignored inputs with `uv run python
...
The preparation step writes 5vyc and 9fqr as protein-only mmCIF so their zsasa
atom workloads match the cleaned PDB inputs; the other six inputs remain native.
It preserves source mmCIF metadata while filtering `_atom_site`, and removes
stale assembly-generation metadata that the pinned FreeSASA parser cannot read.
FreeSASA remains excluded for the two NVDA inputs because it aborts without
producing atomic radii. Interrupted wall-clock runs resume from existing
hyperfine JSON results unless `--replace` is supplied.
